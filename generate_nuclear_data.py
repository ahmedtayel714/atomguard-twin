...
for i in range(NUM_ROWS):
    ...
    if not is_anomaly:
        ...
        radiations[i] = np.random.uniform(0.1, 0.5)    # Radiation mSv/h (New baseline)
        gases[i] = np.random.uniform(0.01, 0.025)       # Hydrogen Gas Concentration %
        humidities[i] = np.random.uniform(40.0, 60.0)  # Humidity % (New)
        pressures[i] = np.random.uniform(100.0, 110.0) # Primary Coolant Pressure kPa (New baseline)
        energy_consumptions[i] = np.random.uniform(35, 45) # Auxiliary Power MW
    else:
        # Anomaly Ranges (spikes or drops)
        anomaly_type = np.random.choice(['high_temp', 'radiation_leak', 'gas_leak', 'high_humidity', 'high_pressure'])
        
        # Base normal values
        temperatures[i] = np.random.uniform(290, 315)
        radiations[i] = np.random.uniform(0.1, 0.5)
        gases[i] = np.random.uniform(0.0, 0.2)
        humidities[i] = np.random.uniform(40.0, 60.0)
        pressures[i] = np.random.uniform(100.0, 110.0)
        ...
        if anomaly_type == 'high_temp':
            ...
            pressures[i] = np.random.uniform(112.0, 118.0) # Pressure rises with temp
        elif anomaly_type == 'radiation_leak':
            radiations[i] = np.random.uniform(1.6, 12.0)  # Significant radiation spike
        elif anomaly_type == 'gas_leak':
            gases[i] = np.random.uniform(1.0, 4.0)        # High hydrogen
        elif anomaly_type == 'high_humidity':
            humidities[i] = np.random.uniform(66.0, 85.0)  # High humidity
            energy_consumptions[i] = np.random.uniform(45, 60) # Power surge due to mechanical stress
        elif anomaly_type == 'high_pressure':
            pressures[i] = np.random.uniform(116.0, 135.0) # Pressure anomaly scenario
            temperatures[i] = np.random.uniform(320, 350) 
